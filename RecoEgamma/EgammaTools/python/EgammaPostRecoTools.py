...
###This determines the format of the scales and smearing file to be read. Before UL, it was of the type ECALELF. Since UL, the format of S&S files changed to make it more compatible so that it can be read in GTs - so essentially this is the first step towards making it read in the GTs
###3 is the ECALELF type and 4 is the TABLE type
def _getEnergySmearingType(era):
    _check_valid_era(era)
    if era=="2017-Nov17ReReco":
        return 3
    if era=="2016-Legacy":
...
def _setupEgammaEnergyCorrections(eleSrc,phoSrc,cfg):
    ...
    energySmearingType = _getEnergySmearingType(cfg.era)
    eleCalibProd.smearingType = energySmearingType
    phoCalibProd.smearingType = energySmearingType
