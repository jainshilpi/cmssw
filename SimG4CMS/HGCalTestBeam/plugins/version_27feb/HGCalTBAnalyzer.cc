...
private:
  ...
  ///SJ
  std::vector<float>                        simHitCellTimeEE_, simHitCellTimeFH_, simHitCellTimeBH_, simHitCellTimeBeam_;
...
void HGCalTBAnalyzer::beginJob() {
  ...
  if (doSimHits_ && doTree_) {
    ...
    if (doTreeCell_) {
      ...
      tree_->Branch("simHitCellTimeEE", &simHitCellTimeEE_);
      tree_->Branch("simHitCellTimeFH", &simHitCellTimeFH_);
      tree_->Branch("simHitCellTimeBH", &simHitCellTimeBH_);
      tree_->Branch("simHitCellTimeBeam", &simHitCellTimeBeam_);
...
void HGCalTBAnalyzer::analyzeSimHits (int type, std::vector<PCaloHit>& hits,
  ...
  std::map<uint32_t,double>                 map_hits, map_hitn, map_hittime;
