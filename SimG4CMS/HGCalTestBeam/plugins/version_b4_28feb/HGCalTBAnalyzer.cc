...
void HGCalTBAnalyzer::analyzeSimHits (int type, std::vector<PCaloHit>& hits,
  ...
  for (auto itr : map_hitTimeEn) {
    ...
    double totE = 0;
    double mindiffEn = 999;
    for( unsigned int ihit=0; ihit<map_hitTimeEn[id].size(); ihit++){
      
      double energy = (map_hitTimeEn[id].at(ihit)).second;
      totE += energy;

      double time = (map_hitTimeEn[id].at(ihit)).first;

      double diffEn = fabs(15-totE/GeV2Mip[id]);
      if(debug){
	std::cout<<"ihit : energy : time : totE till now : totE in mips  "<<ihit<<" "<<energy<<" "<<time<<" "<<totE<<" "<<totE/GeV2Mip[id]<<std::endl;
      }
      
      if(diffEn < mindiffEn){
	mindiffEn = diffEn;
	map_hittime_15Mip[id] = time;
      } //else break;
